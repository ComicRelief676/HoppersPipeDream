...
* designated color
******************************************************************************/
void debugPrint(std::vector<std::string> item, std::string itemColor = "red", std::string desc = "debugPrint:", std::string descColor = "white") {
	color(descColor);
...
******************************************************************************/
void debugPrint(std::vector<int> item, std::string itemColor = "red", std::string desc = "debugPrint:", std::string descColor = "white") {
	color(descColor);
	std::cout << desc << std::endl;
	color(itemColor);
	displayFile(item);
...
/******************************************************************************
* bracketChecker(vector<string> input)
*	I don't know what this function does but I assume it's related to checking
* what lines have brackets on them, either opening or closing
* 
* BECKETT, TAKE IT AWAY
* 
* ...by that I mean finish this comment block
* 
* also, where's the pseudocode my man? We're gonna get flagged by our own
* program for no having a high enough comment/code ratio
******************************************************************************/
std::vector<std::string> bracketChecker(std::vector<std::string> input) {
	std::vector<std::string> brackets;
	bool isCommented = false;
	for (int i = 0; i < input.size(); i++) {
		isCommented = false;
		for (int j = 0; j < input[i].length(); j++) {
			/*if (input[i].length() > 1 && j < input[i].length() - 1) {
				if (input[i].substr(i, 2) == "//") {
					isCommented = true;
				}
			}*/
			if (input[i].at(j) == '{' || input[i].at(j) == '}' && !isCommented) {
...
int main() {
	...
	//////////////////////////////////////////////////////THIS IS WHERE YOU ENDED AIDAN
	displayFile(unnestedFile);
	displayFile(bracketChecker(unnestedFile));

	displayFile(functionCommentBlocks(functionLines, commentBlockStart, commentBlockEnd, unnestedFile));
